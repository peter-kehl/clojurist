...
(def squares-sq (fn [num-from num-max]
  (let [nums (take-while #(<= % num-max)
        ...
        fill-in (fn [places [x y :as place] value] {:pre [(matrix? places) (place? place) (entry? value)]} ;like assoc-in, but using new-map
        ...
        places (let [[places-for-digits place-last-digit dir-last-digit num-of-turns]
                     ...
                     _ (if (< 1 (count digits))
                           ...
                           places-for-digits)])
        _ (do (println :places)(clojure.pprint/print-table places))
