...
; 2. Change from parens-flat to parens-flatter: Move applying rightmost consecutive closers & openers into separate loop. That eliminates a complex conditional.
; 3. NOT IMPLEMENTED: Change from parens-flat to parens-flatter: Move code that generates consecutive the bits part of new rightmost consecutive openers & closers to the first loop.  
...
(if (resolve 'time-check) ;from shell: clj -e "(def time-check true)" src/clojurist/
  (println "time for n=12 (which generates " (time (count (parens-flatter 12))) "results"))
;------
(def parens-flat
  ; - prev: the last result
  ; binary 1 for an opener (, 0 for a closer )
